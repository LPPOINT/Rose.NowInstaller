...
namespace Rose.NowInstaller.Core.Common
{
    ...
    public struct ByteSize : IComparable<ByteSize>, IEquatable<ByteSize>
    {
        ...
        //  than or equal to one.
        /// </summary>
        public string ToString(int afterDat)
        {
            var value = Math.Round(LargestWholeNumberValue, afterDat);


            return string.Format("{0} {1}", value, this.LargestWholeNumberSymbol);
        }
        ...
        {
            return ToString(bytes, 2);

        }

        public static string ToString(double bytes, int afterDat)
        {
            var v = new ByteSize(bytes);
            return v.ToString(afterDat);
        }
